...
pub static ROOT_EXECUTOR: Mutex<Executor> = Mutex::new(Executor::new());
...
impl Executor {
    ...
    pub fn run(executor: &Mutex<Self>) {
        crate::info!("Executor starts running...");
        loop {
            let mut executor_locked = executor.lock();
            let task = executor_locked.task_queue().pop_front();
            if let Some(mut task) = task {
                let waker = no_op_waker();
                let mut context = Context::from_waker(&waker);
                match task.poll(&mut context) {
                    Poll::Ready(result) => {
                        crate::info!("Task completed! {:?}", result);
                    }
                    Poll::Pending => {
